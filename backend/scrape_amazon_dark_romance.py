...
MASTER_STATE_FILE = r"e:\Internship\PocketFM\backend\master_mission_state_amazon_only.json"
MISSION_KEY = "dark_romance"
OUTPUT_FILE = r"e:\Internship\PocketFM\Amazon_Scraping_Dark_Romance.xlsx"

BATCH_SIZE = 50
MAX_TABS = 5
SEARCH_URL = "https://www.amazon.com/s?k=Dark+Romance&i=stripbooks"
    ...
    if os.path.exists(MASTER_STATE_FILE):
        with open(MASTER_STATE_FILE, 'r') as f:
            ...
            if MISSION_KEY in master:
                ...
                return state
    return {"last_page_scanned": 0, "total_processed_global": 0}
        ...
        while int(state['total_processed_global']) < MISSION_TARGET:
            ...
            try:
                ...
                for i in range(0, len(all_discovery_links), MAX_TABS):
                    ...
                    tasks = [asyncio.wait_for(process_book(context, book), timeout=90) for book in batch]
                ...
                if final_rows:
                    ...
                    save_state(state)
                    print(f"Batch saved! Total Processed: {state['total_processed_global']}/{MISSION_TARGET}", flush=True)
