...
async def run_scrape_process(url, limit):
    ...
    print(f"Part 2: Visiting {len(book_list)} pages in a single browser with 15 tabs...")
        ...
        async def limited_tab_scrape(book):
            async with sem:
                try:
                    ...
                    # Sanitize for terminal printing to avoid Unicode crashes on Windows
                    print(f"Opening tab for: {title.encode('ascii', 'ignore').decode('ascii')}")
                    details = await scraper.scrape_product_details_tab(context, book.get('Amazon URL', ''))
                    return {**book, **details}
