...
async def process_row(index, row, df, context, sem):
    async with sem:
        ...
        has_pages = str(num_pages) not in ['0', '0.0', 'nan', 'None', ''] and pd.notna(num_pages)
        
        if has_books and has_pages:
            # We ONLY process if number of pages or books is missing/zero
            return

        print(f"\n--- Processing Row {index + 1}: URL present, but missing pages/books ---")
                    ...
                    if b_link:
                        ...
                        try:
                            ...
                            if extracted_pages:
                                ...
                                print(f"[{index}] Book {idx+1} Pages: Not found (Audiobook/Missing)")
...
async def run_scraper():
    ...
    for batch_start in range(START_ROW, total_to_process, BATCH_SIZE):
        ...
        # Check if we need to launch browser for this batch
        ...
        for i in range(batch_start, batch_end):
            ...
            if has_url:
                has_books = str(num_books) not in ['0', '0.0', 'nan', 'None', ''] and pd.notna(num_books)
                has_pages = str(num_pages) not in ['0', '0.0', 'nan', 'None', ''] and pd.notna(num_pages)
                if not (has_books and has_pages):
                    needs_processing = True
                    break
                    
        if not needs_processing:
            print(f"Batch {batch_start}-{batch_end} requires no updates.")
            continue
